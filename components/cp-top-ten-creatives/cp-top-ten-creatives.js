 ...
 */
export default function CpTopCreativesList({ cards: propCards, onImageClick }) {
  const defaultCards = [
            ...
            <div className="rounded-xl p-6 absolute bottom-0 left-0 right-0 bg-[linear-gradient(126.16deg,rgba(82,82,82,0.24)_-3.39%,rgba(0,30,41,0.4)_58.7%)]">
              <div className="flex justify-between items-center">
                <div className="flex flex-col justify-between pt-4">
                  ...
                  <div className="absolute bottom-11 right-4 text-[96px] font-extrabold leading-none 
                    bg-clip-text text-transparent
                    bg-[linear-gradient(268.87deg,#EC3228_4.49%,#F66B34_86.06%,#FF9B3F_126.9%)] opacity-70">{c.id}</div>
                </div>
              </div>
            </div>
          </div>
